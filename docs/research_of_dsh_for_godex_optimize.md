...
### P2：完善 Agent Engine 接入 ✅ 第 1、2、3、4、5 项已落地
...
4. 统一 text delta、tool、usage、error、permission 等事件映射 —— ✅ 部分：ACP 外部 engine 的 session/update 事件（message chunk → `assistant_text_delta`、tool_call → `tool_call_started/finished`）由 `ACPHarness.emitUpdateEvents` 重放为 GoDex 事件（`tools.ACPUpdate`/`ACPRunResult.UpdateEvents` 结构化解析）；error 映射已落地（失败 turn 发 `error_raised`，`emitErrorEvent`），usage/permission 映射待补；
