...
## 3.5 沙箱节点（供其他节点远程执行工具）

普通节点接入后，其他节点只能通过中心做 `node exec/forward`（跳板）。若希望**本节点成为执行沙箱**——让内网节点 A 把它的 bash/文件工具跑到本节点 B 上（RemoteSandbox，见 `docs/remote-sandbox-design.md`）——接入时带上 `--sandbox-exec-on`：

```bash
godex node join 'https://center' --id pod-b --credential ck_xxx --trust guarded-remote \
  --sandbox-exec-on   # 启用 /control/sandbox/exec|fs，A 可经中心 relay 在此运行工具
```

- **幂等 + 可恢复**：本命令自包含（join + 写 `.env` 凭证 + 启用沙箱端点）。pod 重建后**重跑同一条命令**即恢复全部配置，A 无需任何重配即可继续对话。
