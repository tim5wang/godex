...
| [self-knowledge-design.md](./self-knowledge-design.md) | 自我认知：功能点以 `// godex-feature:` 注释为单一事实源，go:generate 提取编译进二进制，`godex_docs` 工具 / `godex docs` CLI 自查 | Implemented（v1.4 落地，40 个功能点覆盖 README 全部核心特性） |
...
| [business-flow-runtime-design.md](./business-flow-runtime-design.md) | Business Agents 四层混合智能 Flow 运行时设计（固定编排→低成本决策→LLM 兜底→人工），Flow Spec v1、F0–F4 分期 | F0 内核原语（decision 节点 / RetryPolicy / 扩展 when）已落地，见文档 §13；F1–F4 为 Planned |
