...
- **Android 运行时恢复**：git / terminal / slash-shell 在 MIUI/HyperOS 上重新可用。根因：app 进程的 seccomp filter 在启动时杀掉捆绑的 glibc 静态 busybox（SIGSYS），而系统 bionic 二进制与 musl 静态 git 不受影响。移动端运行时现把 shell/工具 applet 符号链接指向系统 bionic 二进制（`/system/bin/sh`、`/system/bin/toybox`），git 及其传输 helper（`git-upload-pack`/`git-receive-pack`/`git-upload-archive`）保留在 `libgit.so`。
- **Relay 会话面板**：relay 会话（`exec_mode=relay:<node>`）的 Files/Terminal 面板现解析执行节点自己的工作目录（而非本地宿主路径），经中心 proxy 路由（6c809ea）。
- **Taskboard 插件（M1–M3）**：五列看板、PJM 编排者（内置专属模板）、按模板分发卡片到独立执行会话、并行冲突四道闸门、执行可观测性（账本一致性 + 失败/卡点洞察 + 追加消息恢复）。
- **Agent 模板市场（M1–M2）**：`/agents` 模板中心、新建对话模板选择器、精确工具集激活（`SetActiveToolsExact`）、memory 模式（none/shared/scoped）、表单化 options 端点。
- **gbash 评估 + 上游 PR**：评估 `ewhauser/gbash` 作为禁 fork Android 设备的零 fork shell 运行时；在 tim5wang fork 准备上游 PR（大输出 spill 到磁盘 + 显式 `TimedOut` 结果语义）。
