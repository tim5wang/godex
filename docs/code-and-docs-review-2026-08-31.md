...
HTTP composition root 已完成三批增量拆分：config、runtime service、control node、provider、automation、memory、notes、channel/weixin、model/security、package、prompt/command 共 72 条路由迁入 13 个窄 registrar；`NewHandlerWithRuntime` 从约 1,883 行降到约 1,009 行，图复杂度从 266 降到 139、cognitive 从 293 降到 160。新增静态 route ownership 测试，禁止生产路由字面量被多个文件重复注册；该项仍为进行中，OpenAI gateway 与 session/turn 等域尚待继续拆分。

## 1. 工具选择与成本控制

主工具选用仓库已经接入的 **codebase-memory-mcp**，不再引入第二套长期索引。最终核验时索引精确指向本仓库，状态 `ready`，包含 15,991 个节点、103,494 条关系，`parse_partial=0`、`skipped=0`。未索引内容是 `.git`、构建产物、截图、node_modules 和 wasm 二进制，不影响源代码结构结论。
...
后续状态：config、runtime service/control node、provider、automation、memory、notes、channel/weixin、model/security、package、prompt/command registrar 与 route ownership 测试已落地；剩余域和窄 `Dependencies` 尚未完成。
...
3. 拆 `NewHandlerWithRuntime` 和 `setStoredValue`，要求行为零变化、先小 registrar/table 后抽接口。HTTP registrar 已完成三批，累计迁移 72 条路由，其余仍进行中。
