...
func (s *SQLiteStore) ListBizKeys() ([]BizAPIKey, error) {
	rows, err := s.db.Query(`SELECT id, name, description, default_prompt, template_id, flow_id, key_prefix, enabled, mcp_servers, providers, sandbox_tools, skills, packages, models, project_dir, budget_credits, warning_threshold, created_at, updated_at FROM biz_keys ORDER BY created_at`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&key.ID, &key.Name, &key.Description, &key.DefaultPrompt, &key.TemplateID, &key.FlowID, &key.KeyPrefix, &enabled,
...
func (s *SQLiteStore) getBizKey(where string, arg string) (*BizAPIKey, error) {
	row := s.db.QueryRow(`SELECT id, name, description, default_prompt, template_id, flow_id, key_hash, key_prefix, enabled, mcp_servers, providers, sandbox_tools, skills, packages, models, project_dir, secret_encrypted, pin_hash, budget_credits, warning_threshold, created_at, updated_at FROM biz_keys WHERE `+where, arg)
	return scanBizKeyRow(row)
}

func (s *SQLiteStore) CreateBizKey(key *BizAPIKey) error {
	_, err := s.db.Exec(`INSERT INTO biz_keys (id, name, description, default_prompt, template_id, flow_id, key_hash, key_prefix, enabled, mcp_servers, providers, sandbox_tools, skills, packages, models, project_dir, secret_encrypted, pin_hash, budget_credits, warning_threshold, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		key.ID, key.Name, key.Description, key.DefaultPrompt, key.TemplateID, key.FlowID, key.KeyHash, key.KeyPrefix, boolInt(key.Enabled),
...
func (s *SQLiteStore) UpdateBizKey(key *BizAPIKey) error {
	result, err := s.db.Exec(`UPDATE biz_keys SET name = ?, description = ?, default_prompt = ?, template_id = ?, flow_id = ?, key_hash = ?, key_prefix = ?, enabled = ?, mcp_servers = ?, providers = ?, sandbox_tools = ?, skills = ?, packages = ?, models = ?, project_dir = ?, secret_encrypted = ?, pin_hash = ?, budget_credits = ?, warning_threshold = ?, updated_at = ? WHERE id = ?`,
		key.Name, key.Description, key.DefaultPrompt, key.TemplateID, key.FlowID, key.KeyHash, key.KeyPrefix, boolInt(key.Enabled),
		encodeStringSlice(key.MCPServers), encodeProviderRefs(key.Providers), encodeStringSlice(key.SandboxTools), encodeStringSlice(key.Skills), encodeStringSlice(key.Packages), encodeStringSlice(key.Models),
...
func scanBizKeyRow(scanner interface{ Scan(dest ...any) error }) (*BizAPIKey, error) {
	...
	var enabled int
	var mcpServers, providers, sandboxTools, skills, packages, models, created, updated string
	err := scanner.Scan(&key.ID, &key.Name, &key.Description, &key.DefaultPrompt, &key.TemplateID, &key.FlowID, &key.KeyHash, &key.KeyPrefix, &enabled,
		&mcpServers, &providers, &sandboxTools, &skills, &packages, &models,
