...
> 状态：Historical / Mostly implemented（3 个 Go 主文件已完成同包拆分；ChatPage 部分完成；新的大文件热点需另立后续债务）
> 日期：2026-08-05
...
| 原目标 | 原行数 | 当前主文件 | 结论 |
|---|---:|---:|---|
| `backend/backend.go` | 7269 | 488 | ✅ 已拆；session/turn/load/persist/helpers/权限/模板/TaskBoard 等已分文件。 |
| `agent/subagent_jobs.go` | 3909 | 335 | ✅ 已拆；store/run/merge/policy/events/git/tool/view 已分文件。 |
| `core/config/manager.go` | 4082 | 394 | ✅ 主文件已拆；但 `resolve.go` 1224、`values.go` 1011 成为新热点。 |
| `ChatPage.tsx` | 3640 | 1919 | 🟡 面板和工具函数已拆出一部分，但未达到原验收的约 1400 行。 |

原计划的“纯移动”工作基本结束；后续不应继续机械套用旧行号区间。新的目标应围绕 `ChatPage`、`backend/turn.go`、`backend/session.go`、`config/resolve.go`、`config/values.go` 的职责边界单独评审。
...
- 当前结果为 488 / 335 / 394 / 1919：前三项达到目标，ChatPage 未达到约 1400 的目标。
- `go build ./...`、受影响包 `go test`、`npx tsc -b` 全绿。
- 拆分后行为零变化：现有测试全部通过即视为行为等价（纯 move，无重构）。
- Go 拆分文件按职责命名；新出现的 >1000 行文件说明“拆主文件”不等于技术债已经消失，需继续做职责级审查而不是只追求行数。
