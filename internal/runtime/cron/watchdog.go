...
// watchdogResult describes the outcome of running a cron watchdog script.
type watchdogResult struct {
	// Skipped is true when the script exited non-zero, meaning the agent run
	// should be skipped for this cron tick.
	Skipped bool
...
// means "run the agent"; a non-zero exit code means "skip this run" (zero token
// cost). Errors (missing script, timeout, exec failure) are returned so the
// caller records the run as failed rather than silently skipped.
//
// The semantics intentionally mirror internal/runtime/heartbeat/watchdog.go so
// cron jobs can reuse the same lightweight pre-run gate.
