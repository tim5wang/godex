...
> **上下文压缩策略（对齐 DSH）**：自动压缩默认 **hybrid**（LLM 摘要 + 规则兜底），阈值与保留尾按模型窗口缩放——`agent.compaction.context_window_tokens`（默认 128000）× `trigger_ratio`（默认 0.8）为压缩阈值；压缩时**只把旧区浓缩成一个 summary 节点，最近的 `retain_ratio`（默认 0.16）即约 20K token 的保留尾逐字节原样保留**（工具输出不再被截断，压缩后无需重新扫描代码）；`retain_tokens`/`trigger_tokens` 可显式覆盖。LLM 摘要调用复用会话自己的 system + 前缀消息（前缀对齐，命中 provider 缓存）。同步压缩受 `max_latency_ms`（默认 3000）约束，超时自动回退规则摘要；后台压缩不受限。**Phase 4**：LLM 摘要输入中的超大 tool result 由模型无关 pruner 裁成"头尾+标记"（`prune_threshold_chars` 默认 8192）；provider 返回 context-length 400 时自动**压缩并重试**（有界 1 次）；`model_policies` 可按 provider/model 覆盖窗口/比例/保留（前缀匹配）。配置详见 `docs/compaction-optimization-plan.md`。
...
| `mcp` | `list_mcp_resources`、`read_mcp_resource`（只读文件系统资源）；`list_mcp_tools`、`call_mcp_tool`、`list_mcp_prompts`、`get_mcp_prompt`（stdio / Streamable HTTP MCP client，tools/prompts 面） | bundle（默认关闭） |
...
- `type: streamable-http`：远端 JSON-RPC MCP server；支持 JSON 与 SSE 终态响应，网络/5xx 最多尝试 3 次，单次 client timeout 10s。当前是无状态 baseline，`session_required` 不会维护 `Mcp-Session-Id`。
